...
When you need to copy data, use clone() to make a modifiable copy (only for a mutable value — on an already-readonly value clone() returns the same immutable value, so use cloneWithType() instead):
\`\`\`ballerina
int[] original = [1, 2, 3];
