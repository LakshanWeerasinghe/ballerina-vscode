 ...
 * model: its listener(s) (with init parameters), service object types (with their remote/resource
 * functions), and declared annotations. Facts only -- no labels, defaults, or curation. A
 * {@code TriggerModel} synthesizer combines these with a connector's {@link TriggerAuthoringModel}
 * to fill in what the authoring-rules document only references by name (the governing DRY
 * principle behind both documents: introspectable facts live here, never restated there).
 *
...
public record TriggerLibraryFacts(List<Listener> listeners, List<ServiceType> serviceTypes,
    ...
    /**
     * A listener class and its init parameters.
     *
